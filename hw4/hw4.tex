...
\lecturedate{March 26}			% required (omit year)
\maketitle
...
\begin{enumerate}[label=(\arabic*)]
    \item S $\to$ *SS $\to$ *aS $\to$ *a+SS $\to$ *a++SSS $\to$ *a++aSS $\to$ *a++aaS $\to$ *a++aaa
    \item S $\to$ *SS $\to$ *S+SS $\to$ *S+Sa $\to$ *S++SSa $\to$ *S++Saa $\to$ *S++aaa $\to$ *a++aaa
    \item 该文法生成的是以a为终结符、带乘法和加法的前缀表达式
    \item 这个文法没有二义性。
    
    \begin{proof}
        每次派生时，子句的前缀只有三种情况(+/*/a)，因此进行子句分割时只需观察其后字符串，递归地进行划分，可以得到唯一语法树。
    \end{proof}
\end{enumerate}
...
\begin{enumerate}[label=(\arabic*)]
    \item 该文法没有左公因子。
    \item 不能，因为文法中存在左递归，不适用自顶向下分析。
    \item 消除左递归之后的文法如下：
    
    \begin{align*}
        rexpr &\to rterm \,\ rexpr' \\
        rexpr' &\to + \,\ rterm \,\ rexpr' \,\ | \,\ \varepsilon \\
        rterm &\to rfactor \,\ rterm' \\
        rterm' &\to rfactor \,\ rterm' \,\ | \,\ \varepsilon \\
        rfactor &\to rprimary \,\ rfactor' \\
        rfactor' &\to rfactor* \,\ rfactor' \,\ | \,\ \varepsilon \\
        rprimary &\to a \,\ | \,\ b 
    \end{align*}

    \item 消除左递归后，现在适用于自顶向下语法分析。
\end{enumerate}
\end{document}
